...
slug: monitor-lock
---
# Java Threads & Monitor Locks 

# 🔒 Monitor Lock (Intrinsic Lock) — Deep Dive
...
* When a thread enters `synchronized(obj) { ... }` it must first **acquire** `obj`’s monitor.
* Only the owning thread can execute the guarded code; others are blocked until the lock is released.

💡 *Analogy:* think of each object as a room with one key. A thread must hold the key to enter the room.
...
If two threads call `increment()` concurrently, both may read the same value and write one increment — losing one update. Synchronizing prevents this:
...
## Lock States & Transitions (HotSpot semantics — conceptual)

HotSpot implements several lock states to optimize for the common case of uncontended synchronization:

1. **Unlocked / No Lock** — object header stores normal Mark Word.
2. **Biased Locking** — a thread may acquire the lock by biasing it to itself without atomic operations. Fastest for single-threaded access.
3. **Lightweight (Thin) Lock** — uses lock records on the owning thread's stack and CAS on the Mark Word for fast uncontended locking.
4. **Inflated (Heavyweight) Monitor** — under contention, the JVM inflates the lock to a monitor object (monitor = kernel-based or VM-managed structure) and blocks other threads (slow path).

**Transition triggers:** contention, unlocking by a different thread, or explicit VM flags.

💡 *Tip:* These optimizations target performance; they are invisible at the language level but affect performance characteristics (latency and throughput) of synchronized sections.

---

## `monitorenter` / `monitorexit` — Bytecode & Compiler Behaviour
...
**Why the duplication and local store?** Because `monitorexit` needs the same object reference on the stack. If an exception occurs inside the block, the catch/finally-like handler uses the stored reference to call `monitorexit` and then rethrows the exception — guaranteeing the lock release.

⚙️ *Compiler guarantee:* The JVM ensures **monitorexit** runs for every successful `monitorenter`, preventing permanent lock ownership after an exception.

---

## `wait()` / `notify()` / `notifyAll()` — Interaction with Monitor

These methods are defined on `Object` and are part of the monitor/condition mechanism — **not** the `Thread` class.
